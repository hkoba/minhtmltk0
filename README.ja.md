...
発火のルールはブラウザに倣っています。グローバルハンドラは他のハンドラの
有無に関係なくイベント 1 回につき 1 回だけ呼ばれ、`node` には最内の
ターゲットが入ります。タグ名や `tag.class` で登録したハンドラ
(`node event on h2 click ...`)はバブリングし、一致する祖先ごとに
1 回ずつ呼ばれます。特定ノードに登録したハンドラは、そのノードの
